...
class SupabaseManager {
    ...
    suspend fun checkRebootCommand(deviceId: String): Boolean = withContext(Dispatchers.IO) {
        try {
            Log.d(TAG, "=== Verificando comando no Supabase ===")
            Log.d(TAG, "Device ID procurado: '$deviceId'")
            Log.d(TAG, "URL Supabase: $SUPABASE_URL")
            
            Log.d(TAG, "Executando query: SELECT * FROM reboot_commands WHERE device_id='$deviceId' AND should_reboot=true AND executed=false")
            ...
            Log.d(TAG, "✅ Comando encontrado! ID: ${response.id}, device_id: ${response.device_id}")
            Log.d(TAG, "Comando: should_reboot=${response.should_reboot}, executed=${response.executed}")
            true
        } catch (e: Exception) {
            // Se não encontrar nenhum comando, retorna false
            val errorMsg = e.message ?: "Sem mensagem"
            Log.d(TAG, "Erro capturado: $errorMsg")
            
            if (errorMsg.contains("No rows", ignoreCase = true) || 
                errorMsg.contains("not found", ignoreCase = true) ||
                errorMsg.contains("PGRST116", ignoreCase = true)) {
                Log.d(TAG, "ℹ️ Nenhum comando de reiniciar pendente para device_id: '$deviceId'")
                Log.d(TAG, "Verifique se o Device ID está correto no Supabase")
                false
            } else {
                Log.e(TAG, "❌ Erro ao verificar comando no Supabase", e)
                Log.e(TAG, "Tipo de erro: ${e.javaClass.simpleName}")
                Log.e(TAG, "Mensagem: $errorMsg")
